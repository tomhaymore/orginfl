...
"""
Runs a series of regular expressions on v in an attempt to clean data
"""
def normalizeOrgs(v):
	#Character Removal/Clean Up
    v = re.sub(r'[\.\?\'\"!\$\*\+\)\(,;#]',"",v) # combined previous regex into one
    v = re.sub(r'\s&\s', " and ", v) # need to distinguish if ampersand has whitespaces around it or not
    v = re.sub(r'&', " and ", v) 
    v = re.sub(r'\s\s+', " ", v)
