...
CREATE INDEX raw_contracts_org_id_idx ON raw_contracts (org_id);
...
FROM raw_contracts WHERE vendoralternatename IS NOT NULL AND org_id IS NOT NULL
    AND org_id > @lastOrgId
UNION -- ALL
SELECT org_id, vendorlegalorganizationname
FROM raw_contracts WHERE vendorlegalorganizationname IS NOT NULL AND org_id IS NOT NULL
    AND org_id > @lastOrgId
UNION -- ALL
SELECT org_id, vendordoingasbusinessname
FROM raw_contracts WHERE vendordoingasbusinessname IS NOT NULL AND org_id IS NOT NULL
    AND org_id >@lastOrgId;

-- Process DUNS:
INSERT IGNORE orgs_refs ( org_id, duns)
SELECT org_id, dunsnumber FROM raw_contracts
WHERE org_id IS NOT NULL AND dunsnumber IS NOT NULL AND dunsnumber!=''
UNION
SELECT org_id, parentdunsnumber FROM raw_contracts
WHERE org_id IS NOT NULL AND parentdunsnumber IS NOT NULL AND parentdunsnumber!='';

-- Match and link agencies
CREATE INDEX raw_contracts_maj_agency_cat_idx ON raw_contracts( maj_agency_cat);

-- ALTER TABLE raw_contracts DISABLE KEYS;
UPDATE raw_contracts JOIN orgs_agencies AS oa
    ON raw_contracts.maj_agency_cat=oa.org_agency_code
SET raw_contracts.agency_org_id = oa.org_agency_o_id;
...
CREATE INDEX raw_contracts_agency_org_id_idx ON raw_contracts( agency_org_id);
UPDATE raw_contracts JOIN orgs_agencies AS oa
    ON  raw_contracts.maj_agency_cat IS NOT NULL 
        AND oa.code=raw_contracts.maj_agency_cat
        AND raw_contracts.agency_org_id IS NULL
SET raw_contracts.agency_org_id = oa.org_id;
...
CREATE INDEX raw_contracts_affs_idx ON raw_contracts (agency_org_id, org_id);
...
FROM raw_contracts AS r
WHERE agency_org_id  IS NOT NULL AND org_id IS NOT NULL;

ALTER TABLE raw_contracts DROP INDEX raw_contracts_affs_idx;
...
FROM affs AS a JOIN raw_contracts AS r 
